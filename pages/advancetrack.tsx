...
export default function MyComponent() {
    ...
    const center = useMemo(
        () => ({ lat: FetchData.length > 0 ? parseFloat(FetchData[0].latitude) : 0, lng: FetchData.length > 0 ? parseFloat(FetchData[0].longitude) : 0 }),
        [FetchData]
    );
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(http://localhost:8000/api/v1/vehicleData?vehicleNo=${vehicleNo});
                ...
                if (data && Array.isArray(data.selectedVehicle)) {
                    data.selectedVehicle.forEach(object => {
                        if (object.Latitude !== undefined && object.Latitude !== 0 &&
                            object.Longitude !== undefined && object.Longitude !== 0 &&
                            object.Speed !== undefined) {

                            if (String(object.Latitude).length > 7 && String(object.Longitude).length > 7) {
                                const formattedData = {
        ...
        const initMap = async () => {
            try {
                ...
                if (FetchData.length > 0) {
                    ...
                    const snappedRoadPath = await Promise.all(FetchData.map(async (point) => ({
                    ...
                    for (let i = 1; i < snappedRoadPath.length; i++) {
                        ...
                        if (color === currentColor) {
                            ...
                            if (i > 1 && routeColor[i - 1] !== color) {
                                ...
                                google.maps.event.addListener(route, 'mouseover', async (event: any) => {

                                    const hoveredIndex = findHoveredIndex(event, route.getPath().getArray());
    ...
    const findHoveredIndex = (event: any, path: any[]) => {
        ...
        for (let i = 0; i < path.length; i++) {
            const distance = google.maps.geometry.spherical.computeDistanceBetween(latLng, path[i]);
                ...
                {isInfoWindowOpen && (
                    <InfoWindowF
                        ...
                        <div className="w-80 p-2">
                            ...
                            <p>
                                Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptate, dolor nisi accusantium quia tenetur voluptatum. Laudantium suscipit dolores, obcaecati placeat autem voluptas libero aspernatur maiores ex aut, dignissimos quia inventore.
                            </p>
                        </div>
                    </InfoWindowF>
                )}
